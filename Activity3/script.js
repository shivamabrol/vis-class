...
d3.csv('data/disasters.csv')
  .then(data => {  //ARROW function - pass data as a parameter into this anonymous function
	...
	for(let yr = 2017; yr >= 1980; yr--){
		...
		// what parameters do you need to draw the line? 
	}
    ...
    data.forEach(d => { //ARROW function - for each object in the array, pass it as a parameter to this function
		...
		let tokens = d.start.split("-");
  		...
  		let cy = getYearPosition2( year, gapBetweenTimelines, timelineX1); //******* don't use 0, use getYearPosition
  		let radius = mapFromDomainToRange(1, 160, timelineX1, timelineX2, d.cost); //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
  		let color = lookupColor(d.category); //******* don't use gray, use lookupColor

     	// for each event, draw a circle, along the timeline
     	drawCircle( cx, cy, radius/10, color, svg);
  ...
  function getYearPosition2( year, gapBetweenTimelines, startOfTimelinesY ){

	return (2019 - year) * gapBetweenTimelines + startOfTimelinesY; 
